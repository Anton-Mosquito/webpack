let questions = {
  'How many letters are there in the word "Hello?"': "5",
  'How many letters are there in the word "World?"': "5",
  'How many letters are there in the word "Yesterday?"': "9",
};

class TestWithQuestions {
  constructor(questions) {
    this._container = document.querySelector("[data-test2-wrapper]");
  ...
  createInterface() {
    ...
    const arrWithQuestion = Object.keys(this._baseQuestions);
    for (const iterator of arrWithQuestion) {
  ...
  checkForInput(element) {
    ...
    const rightAnswer = this.findRightAnswers(curentIndex, this._baseQuestions);
  ...
  checkForButton(element) {
    ...
    arrElements.forEach((item) => {
      ...
      const rightAnswer = this.findRightAnswers(
        curentIndex,
        this._baseQuestions
      );
